...
const API_KEY_STORAGE = 'kvkk_gemini_api_key';
...
3. İlk 30 Günde Atılması Gereken Adımlar (madde madde, maksimum 5 madde)`;
    ...
    const yorum = data.candidates?.[0]?.content?.parts?.map(p => p.text).join('\n') || 'Boş cevap';

    STATE.aiNotes._genel = yorum;
    saveState();
    document.getElementById('ai-result').innerHTML = `
      <div style="background:linear-gradient(135deg,rgba(66,133,244,0.1),rgba(6,182,212,0.1));border:1px solid rgba(66,133,244,0.3);border-radius:10px;padding:1.25rem;margin-top:1rem">
        <div style="font-size:11px;color:#60a5fa;text-transform:uppercase;letter-spacing:.6px;font-weight:600;margin-bottom:.75rem">🤖 Gemini AI Değerlendirmesi</div>
...
function saveAPIKey() {
  ...
  if (!key.startsWith('AIza') || key.length < 30) { toast('Geçersiz Gemini API anahtarı (AIza ile başlamalı)', 'error'); return; }
